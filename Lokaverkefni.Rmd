...
data_fjolbylishus %>%
  ...
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.background = element_blank()) + 
  ggtitle("Figure 3: Mean property prices for apartment buildings with/without an elevator by area") + 
  xlim(c(0, 45000)) + 
...
colnames(da) <- c("Pr.Price","Area","Floors","Parking","Baths","Showers","Toilets","Kitchens","Rooms","Liv.r.","Storage","Age","Sqm.Price")
corrplot.mixed(cor(da[-12]), lower.col = "#d88121", number.cex=0.7, tl.pos = "lt", upper.col = "#8c5e4a", order="hclust", tl.col="#341e12", tl.cex=0.7)
```
LAGA - title labels
...
data.frame(lambda = lasso.cv$lambda, 
  ...
  ggplot(aes(x = lambda, y = cv.mse, col = ifelse(lambda==bestlam, "#d88121", "#c3987d"))) + geom_point() + 
  geom_line() +
  ...
  labs(x = "Lambda", 
       ...
       title = "10-Fold CV Lasso Lambda Selection")+ 
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), 
        panel.background = element_blank(), legend.position = "none")
```
