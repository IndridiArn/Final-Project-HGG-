...
In figure 1 we see the distribution of property worth based on the municipality on one hand, and neighbourhood on the other. They are mostly right skewed. The average worths in Seltjarnarnes ($\mu approx$ 52 m.ISK), Garðabær ($\mu approx$ 49 m.ISK) are the highest. The lowest average worths are in Hafnarfjörður ($\mu approx$ 35 m.ISK) and Reykjavík ($\mu approx$ 35 m.ISK). 

  The distribution for Seltjarnarnes differs from the rest in both graphs; Left graph shows it is multimodal, and the right shows a flatter curve compared to the other neighbourhoods. The property worths in Flatir neighbourhood in Garðabær is the only left-skewed distribution.   
  
  The most expensive properties on average are based on neighbourhood are in Flatir ($\mu \approx$ `r round(max[2]/1e3,2)` m.ISK) and the least expensive are outside the urbanized area of Kópavogur ($\mu \approx$ `r round(min[2]/1e3,2)` m.ISK). 
...
In figure 2 we can see the property worth by year of sale. Property worths, and their variability, have gradually increased over the years. The average property worth over the years 2012 to 2018 is approximately 37 m.ISK.
...
In figure 3 we see plots of property worth according to various property elements. In general, as elements in a property increase, such as having more baths, showers, toilets and kitchens, the worth increases. Although, the worth does not appear to increase in the same fashion for number of living rooms, storage rooms and rooms.
...
data_fjolbylishus %>%
  ...
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.background = element_blank()) + 
  ggtitle("Figure 4: Mean worths for apartments by municipality and elevator status") + 
...
In figure 4 we see average worth of apartments, in each municipality, based on whether the building has an elevator or not. Apartments in buildings with an elevator are on average more expensive. The percentages on the plot represent the increase in worth for having an elevator in the building. Seltjarnarnes is the only municipality where having an elevator does not increase the propery worth.  
...
ggplot(f3, aes(x=nuvirdi/1e3,y=age, group = ibteg, color = ibteg)) +
  geom_smooth() + 
  ggtitle("Figure 5: Property worth based on age and type of property") +
...
In figure 5 we have the average property worth based on age and type of property. As expected, single-family houses are on average more expensive than apartment buildings and the older the property the more inexpensive it is. Age appears to become irrelevant for single-family houses worthd 50 million isk and above.   
...
In figure 6 we see worth per square meter by municipality. We can gather that worths per square meter are a bit higher for properties in Garðabær and Seltjarnarnes (and Mosfellsbær to a degree) than for properties in the other municipalities. 

```{r}
# fjölbýli vs einbýli eftir municipality. Figure 7
einbyli.perc <- percent(length(data$ibteg[data$ibteg==1])/length(data$ibteg))
fjolbyli.perc <- percent(length(data$ibteg[data$ibteg==0])/length(data$ibteg))
perc.list <- c(fjolbyli.perc,einbyli.perc)

data7 <- data %>% 
  mutate(ibteg = ifelse(data$ibteg == 1, "Single-family house", "Apartment building"))

data7$ibtegPerc <- ifelse(data$ibteg==1,einbyli.perc,fjolbyli.perc)

data7 %>% 
  ggplot(aes(x = ibteg, fill = ibteg)) + 
  geom_bar(stat= "count", position = "dodge") +
  geom_text(aes(ibteg,1, label=ibtegPerc,vjust=-10.8)) +  #Hræðileg hack lausn með því að setja 1 á milli ibteg og label. How Why WHAT
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), 
        panel.background = element_blank()) + 
  ggtitle("Figure 7: Property type by municipality") +
  scale_fill_manual(name = "Property type", values=sample(ColorsDT$Color)) + 
  labs(x = " ", y = "Total")  +  ylim(0, 30000) 
```
...
A correlation plot of 14 property elements can be seen on figure 8. worth of properties appears to have a moderately strong positive correlation with the size of properties (in square meters), year of sale and number of rooms, living rooms, floors, showers and toilets in a property. 
...
In this part we will predict sale worths using the lasso and boosting. 
...
```
  ...
  Running factor analysis on the data set as a whole obtained 25 dimensions all with eigenvalues higher than one, indicating that the dimension accounts for more variance than one of the original variables. These 25 dimensions only collectively explained around 66% of data variance. The variable for neighbourhood had no correlation to any dimensions so it will be removed for further analysis, addition to worth per square meter since it had high correlation to other predictors. 
...
ba %>%
  ...
  labs(x= "Dimensions", y = "Variance of prop. worths explained") +
...
```
  ...
  Removing the two variables reduced the influential dimensions from 25 to seven and only decreased the cumulative variance explained by around 2%. 

```{r}
fviz_contrib(res.famd, choice="var", axes=1:7) + ggtitle("Figure X: Variable contribution")
```

  Here we can see visually how influential the variables are to the data set as a whole. Clearly municipality causes the most variablity compared to the other variables.
...
```
  ...
  Here we can see how the variables influence each dimension. This suggests splitting municipality, which aligns with the previous graph. Additionally it seems like the number of shower and bath a property has might work together, perhaps as number of master bathrooms. Sales Year seems like a strong variable on it's own. 
  
  Only the first seven dimensions had eigenvalues above one. Therefore we might assume that splitting municipality would only make two of it's regions strong predictors. Lastly all the variables included in the first dimension, except for property pricing and property type, might work as a unit as property size.
...
ha <- plot[sapply(plot$value, FUN = function(x) all(x >= 0)),]
ha2 <- plot[sapply(plot$value, FUN = function(x) all(x < 0)),]
...
haha %>%
  ggplot(aes(value,reorder(coef,value),
...
```
  ...
  Figure 9 shows the predictors that Lasso deemed influental regarding property pricing. The bottom seven variables are estimated to result in a decrease in property pricing when their values increase. The remaining variables would have the opposite effect. 

The lasso approach found `r nrow(haha)` important coefficients. The model explained around `r r2train.lasso` of variance ($R^2$) in property worth for the training data and around `r r2.lasso` for the test data. With $\lambda$ set to the optimal CV value, training error was around `r lasso.trainmse` m.ISK and test error around `r lassomse.` m.ISK. 

  In align with the factor analysis result the regions of the municipality variable do seem to work well as seperate predictors with differing influence regarding property worth. Additionally in support of the factor analysis results number of toilets and bathtubs seem to have similar effect on property pricing and might be considered as a singular predictor.
  
  The main issue with this approach is blindness regarding correlated/confounding variables. As noted earlier (see Figure X.) square meter had the strongest influence on data variance. Here we see that variable is included but rather low, however all the variables from the first dimension of the factor analysis are also included here. Since Lasso is blind to their influence on each other it is important to keep that in mind and not exclude the effect that square meter might have on property worth. 
...
As shown in figure 12, there are four variables that have high relative importance. They are square meters of property, the year the property was bought, the number of bathrooms and municipality. Square meters per property is by far the most important variable. In figures 3 and 7 in part I we learned that the more square meters and bathrooms are in a property the more expensive it is. We also learned that property worths increase the younger the property is.
