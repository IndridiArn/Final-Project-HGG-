...
Figure 7 shows the split of property types for each municipality. The plot shows the percentage of apartment buildings. The highest ratio of apartments to single-family houses is in Reykjavík and Kópavogur. 
...
The last five elements mentioned here above also appear to have a moderately strong postive relationship with each other. 
...
LAGA: næ ekki að gera þessa mynd fína!
  ...
  Here we can see how the variables influence each dimension. The plot indicates that we should look at each municipality seperately and aligns with previously seen information (see Figure 10). Additionally, it seems like the number of showers and baths a property has might work together, perhaps as number of master bathrooms. Sales year seems like a strong variable on it's own. 
  
  Only the first seven dimensions had eigenvalues above one. Therefore we might assume that splitting the municipality variable into its factors would only make two of it's factors strong predictors. All variables included in the first dimension, except for property pricing and property type, might indicate a single factor belonging to property size.
...
haha %>%
  ...
  scale_color_manual(values=sample(ColorsDT$Color)) + labs(x="Coefficient", y=" ") + ggtitle(expression(paste("Figure 12: Predictors with non-zero Lasso coefficients with ", lambda, " = 0.36")))
...
lasso.newTrainingMSE <- log10(round(mean((lasso.pred[,1])^2),2))
lasso.newTestMSE <- log10(round(mean((lasso.pred.[,1])^2),2))
```

  Figure 12 shows the `r nrow(haha)` predictors that Lasso deemed influental regarding property worth. The bottom seven variables have a negative relationship with property worth while the remaining have a positive relationship. The variables Seltjarnarnes, number of floors, year the property was bought, Garðabær, Hafnarfjörður and number of garages are the most influential according to the lasso.   

  The model explained around `r r2.lasso` of property worth variance ($R^2$). With training error around log10(`r round(lasso.newTrainingMSE, 2)`) and test error around log10(`r round(lasso.newTestMSE, 2)`).

  In alignment with the factor analysis, the municipality regions do seem influential for property worth as seperate predictors. Additionally, number of toilets and bathtubs seem to have similar effect on property worth and might be considered as a singular predictor as noted earlier.
  
  The main issue with Lasso are correlated/confounding variables. As seen earlier (see Figure 10) square meters (size of property) came in second when it comes to explaining variance in the data. In figure 12 this variable (size) is included but its coefficient has been reduced almost to zero. Meanwhile, all the variables from the first dimension of the factor analysis appear to also be included here. Lasso is blind to the influence of square meter on property worth (due to high correlation) and it is important to keep that in mind and not exclude the effect that square meter has on property worth. 
...
grid.arrange(g2,g1, nrow=1,top="Figure 13: Lasso predicted versus observed property worths", bottom="Predicted", left="Observed")
```

In Figure 13 we see the relationship between lassos' predicted values and observed values. The right graph includes all values in the data set and we appear to have an extreme value at the top of the plot. 
  The left graph is a close-up of all the values excluding the extreme value. The relationship seems stronger for properties of lower worth but starts to gradually worsen as the worth goes up.
...
```
In figure 14 we see a plot of the RMSE via cross-validation according to various measures of tree size and shrinkage value. We can see here that using 500 boosting iterations, with $\lambda$ of 0.1 and a max tree depth of 10 results in the lowest RMSE out of all the tree combinations that were tested. 
...
BOOST.test.MSE <- log10(round(mean((NewBoost.predict - test.set$nuvirdi)^2), 2))
...
In table 1 we can see the results from both the lasso approach and boosting. Boosting outperforms the lasso as the test MSE and test RMSE for boosting is lower. In figure 15 we saw that there were a total of 10 variables mentioned when using the boosting method. Out of those, only 3 of them stand out as influential variables. In figure 12 we saw a total of 19 predictors with non-zero coefficients in the plot. For the sake of interpretation, we have regarded only 13 of those variables as influential based on the coefficients' distance from zero.  


## Part III: Predicting property age (old or new)
...
kbl(kabletable,booktabs=T) %>% 
  kable_styling(full_width = F, position = "center") %>%
  add_header_above(c(" ","True values" = 2),bold=T)  %>% 
  add_header_above(c("Table 2: Quadratic discrimnant analysis" = 3),bold=T,font_size = 16)
...
The test error rate of the QDA model is  `r meanQDA`, meaning the model predicted incorrectly nearly  `r meanQDAperc` of the time. 
...
kbl(kabletable2) %>% 
  ...
  add_header_above(c("Table 3: Logistic Regression" = 3),bold=T,font_size = 16)
...
The test error rate of the logistic regression model is  `r meanLOG`, meaning the model predicted incorrectly nearly  `r meanLOGperc` of the time. Similar result to that of QDA.   
...
old %>%
  kable() %>%
  kable_styling(full_width = F) %>% 
 add_header_above(c("Table 4: Comparison of QDA and LR" = 3),bold=T,font_size = 16)
```
In table 4 we see a comparison of the results between QDA and logistic regression. The two methods have very similar results, although the percentage of false negatives is slightly (still noticably) higher for QDA. 
...
randforr_list_MSE <- c(0.1986711, 0.1682962, 0.1576649, 0.1521595, 0.1497864,0.1489321,0.1482677,0.1463692,0.1483626,0.1464642, 0.1492169, 0.1487423, 0.1510204,0.1506407, 0.1533935) # Breyturnar frá MSE calculations í Cross-Validation.    

min.cv <- which.min(randforr_list_MSE)
best.CV <- which.min(randforr_list_MSE)
plot(randforr_list_MSE, type = "b",xlab = "Degree", ylab = "Test MSE", main="Test MSE by number of variables")
points(best.CV, randforr_list_MSE[best.CV], col = "darkred",pch = 10,cex =30)
```
...
randforr_best <- randomForest(AgeNew~.,data=train.set2,mtry=best.CV,importance=T,ntree=500)
pred_randforr_best <- predict(randforr_best,newdata = test.set2)
...
plot(pred_randforr_best)
  
  
varImpPlot(randforr_best)
randforr_MSE.perc <- percent(randforr_MSE_best)
...
bonusdatafjolbyli <- read.csv("https://github.com/IndridiArn/Final-Project-HGG-/blob/main/bonusdataserbyli.csv")
