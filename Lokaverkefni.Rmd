...
```
  ...
  Figure 10 shows how much data variability variables explain. Clearly municipality explains the most variablity compared to the other variables.
...
LAGA: næ ekki að gera þessa mynd fína!
  ...
  Here we can see how the variables influence each dimension. This suggests splitting municipality, which aligns with previous information (see Figure 10). Additionally it seems like the number of shower and bath a property has might work together, perhaps as number of master bathrooms. Sales year seems like a strong variable on it's own. 
...
```
  ...
  In align with the factor analysis the municipality regions do seem influential for property worth as seperate predictors. Additionally, number of toilets and bathtubs seem to have similar effect on property pricing and might be considered as a singular predictor as noted earlier.
...
data.frame(mse=test.MSE, 
  ...
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), 
        panel.background = element_blank(), legend.position = "none", plot.title = element_text(hjust = 0.5)) + 
  ggtitle("Figure 14: Test MSE for a range of lambda values") + 
  annotate(geom="text",y=10^(10+log10(min.testMSE)), x=10^log10(min.lambda), label= expression(paste(lambda, "= 1.44")))
```

Figure 14 shows the test error based on a range of shrinkage $\lambda$ values. The CV optimal value was `r min.lambda` which obtained the test MSE of `r round(min.testMSE/1e3, 2)` m.isk. 
