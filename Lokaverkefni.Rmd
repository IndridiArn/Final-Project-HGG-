...
```
  ...
  Figure 11 shows the `r nrow(vi(lasso.mod))` predictors that Lasso deemed influental regarding property worth. The model explained around `r r2.lasso` of property worth variance ($R^2$). With training error around `r round(lasso.trainmseNew, 2)` and test error around `r round(lasso.testMSE, 2)`. 
...
#which variables are most important. 
boost2 <- vip(gbm_tree_final, aesthetics = list(fill = c("#d48149","#da9566","#ca6e2f","#ad5e28")),
    mapping = aes_string(fill = "Variable")) + 
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.background = element_blank(),
        plot.title = element_text(hjust = 0.5)) + 
  labs(x = " ", y = "Relative influence") 
boost2 + ggtitle("Figure 18: Relative influence of variables using boosting") 
...
grid.arrange(boostbonus,boostfyrri, nrow=1, top="Figure 19: Boosting comparison") 
```

=======

As shown in figure 16, there are three variables out of the four that have higher relative importance. They are square meters of property, the year the property was bought and the age of the property. Square meters per property is by far the most important variable, which aligns with the results from before. 
>>>>>>> 3e1ba3512416d03db4c4efbfc666aadaf0700e04
