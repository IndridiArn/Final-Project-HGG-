...
```
  ...
  The model explained around `r r2.lasso` of property worth variance ($R^2$). With training error around `r lasso.trainmseNew` m.ISK and test error around `r lassomse.new` m.ISK. 
...
old %>%
  kable() %>%
  kable_styling(full_width = F) %>% 
 add_header_above(c("Table 5: Comparison of QDA, LR and Random forest" = 4), bold=T , font_size = 16)
```
In table 5 we see a comparison of the results between QDA, logistic regression and random forest. Overall, random forest outperforms QDA and logistic regression with the highest percentage of accuracy, true negatives and true positives. QDA and logistic regression have very similar results, although the percentage of false negatives is slightly (but still noticably) higher for QDA.
