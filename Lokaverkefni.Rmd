...
vip(randforr_best, aesthetics = list(fill = c("#d48149","#da9566","#ca6e2f","#ad5e28","#e1a983","#e8bda0","#2e1f18",
  ...
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.background = element_blank(),
        plot.title = element_text(hjust = 0.5)) + 
  ggtitle("Figure X: Relative influence of variables using random forest") + 
  labs(x = " ", y = "Relative influence") 
```
...
randforr_MSE_best <- percent(randforr_MSE_best)
```
...
old %>%
  kable() %>%
  kable_styling(full_width = F) %>% 
 add_header_above(c("Table 4: Comparison of QDA, LR and Random forest" = 3),bold=T,font_size = 16)
```
LAGA TÚLKUN: In table 4 we see a comparison of the results between QDA, logistic regression and random forest. The two methods have very similar results, although the percentage of false negatives is slightly (but still noticably) higher for QDA. 
