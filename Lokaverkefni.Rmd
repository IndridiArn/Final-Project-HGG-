...
```{r, fig.align = 'center'}
colnames(blabb) <- as.list(var.labels[match(names(blabb), names(var.labels))])
...
vi(lasso.mod) %>% ggplot(aes(Importance, reorder(Variable,Importance),
  ...
  scale_fill_manual(values=sample(ColorsDT$Color)) + labs(x="Beta coefficient", y=" ") + ggtitle(expression(paste("Figure 9: Predictors with non-zero Lasso coefficients with ", lambda, " = 0.36")))
...
colnames(datam) <- as.list(var.labels[match(names(datam), names(var.labels))])
...
boost + ggtitle("Figure 15: Relative influence of variables using boosting") 
...
boost <- com %>% ggplot(aes(Importance.y,Variable)) +
  ...
  scale_fill_manual(values=sample(ColorsDT$Color)) + labs(x="Relative Influence", y=" ") + ggtitle("Boosting") + scale_y_discrete(labels=NULL) 

grid.arrange(lasso, boost, nrow=1)

tafla <- data.frame(Boosting <- c(MSE=round(BOOST.test.MSE, 3),
          RMSE=round(rmse.boost, 3),
          Variables="10"),
          Lasso <- c(MSE=round(lasso.testMSE, 2),
           RMSE=round(lasso.RMSE, 2),
           Variables="19"))
...
=======
>>>>>>> 4facd199d847f8e18c3bbcdadc577b1b1a224987
#### Random forests
