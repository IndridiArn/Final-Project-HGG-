...
In figure 4 we have the average property price based on age and type of property. Single-family houses are on average more expensive than apartment buildings and the older the property the more inexpensive it is. Age appears to become irrelevant for single-family houses priced 50 million isk and above.   
...
A correlation plot of 14 property attributes can be seen on figure 6. Price of properties appears to have a moderately strong positive correlation with the size of properties (in square meters), year of sale and number of rooms, living rooms, floors, showers and toilets in a property. 
...
data.frame(lambda = lasso.cv$lambda, 
  ...
  ggtitle("Figure 7: Lasso Cross-Validation") + 
  annotate(geom="text",y=10^8.1438, x=10^0, label= expression(paste(lambda, "=10")))
```

Our optimal value of $\lambda$ is log10(`r round(bestlam,2)`), for minimum test MSE. The test MSE is`r round(bestmse,2)` 
...
haha %>%
  ggplot(aes(value,reorder(rownames(haha),value), col=coef)) + geom_point(show.legend = F, size = 3)+ 
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), 
        panel.background = element_blank(), legend.position = "none") + 
  scale_fill_manual(values = sample(ColorsDT$Color)) + 
  scale_color_manual(name = "Type of property",values=sample(ColorsDT$Color)) + labs(x="Coefficient", y=" ") + ggtitle(expression(paste("Figure 8: Predictors with non-zero Lasso coefficients with ", lambda, " = 10")))
test <- blabb[-train,]
...
The Lasso approach found `r nrow(haha)` important coefficients. The model explained around `r r2train.lasso` of variance ($R^2$) in property price for the training data and around `r r2.lasso` for the test data. With $\lambda$ set to log(10) training error was around `r lasso.trainmse` m.ISK and test error around `r lassomse.` m.ISK. 
...
grid.arrange(g2,g1, nrow=1,top="Figure 9: Lasso predicted versus observed property prices", bottom="Predicted", left="Observed")
```

In Figure 9 we see visually the relationship between the Lasso´s predicted values and the observed values. The right graph includes all values, clearly there is an extreme value at the top. 
  The left graph is a close up to all the values excluding the extreme value. The relationship seems stronger for lower priced property but starts to gradually worsen as the price goes up.
...
min.lambda. <- lambdas[which.min(test.MSE)] # gives the lambda corresponding to lowest test
min.lambda <- round(min.lambda.,2)
data.frame(mse=test.MSE, 
           lambda=lambdas) %>%
  ggplot(aes(x=lambda,y=test.MSE,
             col= ifelse(lambda==min.lambda., "#493021","#d88121"))) +
  geom_point(size=3) + 
  geom_line() +
  scale_x_log10(breaks = trans_breaks("log10", function(x) 10^x),
               labels = trans_format("log10", math_format(10^.x))) +
  ...
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), 
        panel.background = element_blank(), legend.position = "none", plot.title = element_text(hjust = 0.5)) + 
  ggtitle("Figure 10: Test MSE for a range of lambda values") + 
  annotate(geom="text",y=10^25, x=10^-0.1, label= expression(paste(lambda, "= 0.7")))
```
...
SLEPPA því að nefna minimum test MSE? 

```{r, fig.align = 'center'}
boost <- gbm(nuvirdi ~ ., data = train.set,
             distribution = "gaussian", 
             shrinkage = 0.7, # common lambda to use (0.01 or 0.001). BREYTA í BEST LAMBDA
             n.trees = 100)
library(vip)
...
LAGA TÚLKUN. There are four variables that have high relative importance, they are square meters of property, price per square meter, the number of bathrooms and year the property was bought. Square meters per property is by far the most important variable. In figure X and X in part I we learned that the more square meters and bathrooms are in a property the more expensive it is. We also learned that property prices increase the younger the property is.
...
```{r}
tafla <- data.frame(Boosting <- c(MSE=paste0(round(min.testMSE/1e6,2)," m.ISK"),
...
t(tafla) %>%
  kable(align="c", caption = "Table X: Comparison of Boosting and Lasso") %>%
...
Using random Forest 
```{r}
randforr_cs <- randomForest(AgeNew~.,data=train.set2,mtry=9,importance=T,ntree=100) #TODO skoða að breyta Ntree og mögulega mtry, því fleiri sem ntree eru því lengri tíma tekur að keyra. 
pred_randforr <- predict(randforr_cs,newdata = test.set2)
randforr_MSE <- round(1-mean((pred_randforr==test.set2$AgeNew)),2)
plot(randforr_cs) # TODO breyta þessu plotti, ég hélt að þetta myndi ekki virka en viti menn.
#text(randforr_cs,pretty=0)
  
  
varImpPlot(randforr_cs)
