...
data.frame(lambda = lasso.cv$lambda, 
  ...
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), 
        panel.background = element_blank(), legend.position = "none") + ggtitle("Figure 7: Lasso Cross-Validation") + annotate(geom="text",y=10^8.1438, x=10^0, label= expression(paste(lambda, "=10")))
```

Our optimal value of $\lambda$ is log10(`r round(bestlam,2)`), for minimum MSE. 
...
test.set <- data[-TRAIN,]
# Tuning lambda
lambdaGrid <- seq(-2, -0.05, length.out = 20) # Typical values are 0.01 or 0.001
...
data.frame(mse=test.MSE, 
  ...
  scale_y_log10(breaks = trans_breaks("log10", function(x) 10^x),
               labels = trans_format("log10", math_format(10^.x))) +
  scale_color_identity() + xlab( expression(paste("Shrinkage value(", lambda, ")"))) + ylab("Cross-Validation MSE") +
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), 
        panel.background = element_blank(), legend.position = "none") + ggtitle("Figure 10: Test MSE for a range of lambda values") + annotate(geom="text",y=10^7.8, x=10^0, label= expression(paste(lambda, "= 0.79")))
```
ATH: log10(0.79) = -0.1 -- LAGA láta línuna fara í síðasta punktinn
