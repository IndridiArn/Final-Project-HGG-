...
library(Hmisc)
var.labels <- c(nuvirdi = "Property Worth", lyfta="Elevator", ibm2 = "Sqm", fjhaed = "Floors", fjbilsk = "Parking", fjbkar = "Bathtubs", fjsturt = "Showers", fjklos="Toilets", fjeld = "Kitchens", fjherb="Rooms", fjstof = "Living Rooms", fjgeym = "Storage", matssvaedi = "Neighbourhood", ibteg = "Prop. Type", svfn = "Municipality", yearkeypt = "Salesyear", age = "Age", fermetraverd = "Price per Sqm", svfnMosfellsbaer = "Mosfellsbær", svfnHafnarfjordur = "Hafnarfjörður", svfnSeltjarnarnes = "Seltjarnarnes", svfnGardabaer = "Garðabær", svfnKopavogur = "Kópavogur", ibteg1 = "Single-family property", ibteg0 = "Apartment", lyfta1 = "Elevator present", lyfta0 = "No elevator")

var.labels2 <- c(nuvirdi = "Property Worth", lyfta="Elevator", ibm2 = "Sqm", fjhaed = "Floors", fjbilsk = "Parking", fjbkar = "Bathtubs", fjsturt = "Showers", fjklos="Toilets", fjeld = "Kitchens", fjherb="Rooms", fjstof = "Living Rooms", fjgeym = "Storage", matssvaedi = "Neighbourhood", ibteg = "Prop. Type", svfn = " ", yearkeypt = "Salesyear", age = "Age", fermetraverd = "Price per Sqm", svfnMosfellsbaer = "Mosfellsbær", svfnHafnarfjordur = "Hafnarfjörður", svfnSeltjarnarnes = "Seltjarnarnes", svfnGardabaer = "Garðabær", svfnKopavogur = "Kópavogur", ibteg1 = "Single-family property", ibteg0 = "Apartment", lyfta1 = "Elevator present", lyfta0 = "No elevator")
```
...
Figure 2 shows a gradual increase in property worths, and their variability, over the years. The average property worth over the years 2012 to 2018 is approximately 37 m.ISK. 
...
Figure 3 shows property worth based on various elements. In general, these elements seem to have a positive linear relationship with property worth. Where number of living rooms, storage rooms and rooms, seem to be an exception the relationship between these variables has a clear curve.
...
Figure 6 shows property worth per square meter by municipality. Property worth per square meter is a bit higher for properties in Garðabær and Seltjarnarnes (and Mosfellsbær to a degree) than it is for properties in the other municipalities. 
...
haha <- plot[-1,] %>%
  ...
  filter(Value>0) %>%
  arrange(-Value)
haha <- data.frame(t(haha))
library(janitor)
haha <- haha %>% row_to_names(row_number=1)
colnames(haha) <- as.list(var.labels[match(names(haha), names(var.labels))])
haha <- t(haha)
...
haha %>%
  ...
  ggplot(aes(Value, reorder(rownames(haha),Value),
             col = col)) +
  geom_point(size = 3)+ 
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), 
        panel.background = element_blank(), legend.position = "top") + 
  scale_color_manual(values=sample(ColorsDT$Color)) + labs(x="Coefficient", y=" ", col="Relationship with property worth:") + ggtitle(expression(paste("Figure 9: Predictors with non-zero Lasso coefficients with ", lambda, " = 0.36")))
...
colnames(datam) <- as.list(var.labels2[match(names(datam), names(var.labels2))])
...
gbm_tree_final <- train(`Property Worth` ~ ., data = train.set, 
                        method="gbm", 
...
```
In table 4 we see a comparison of the results between QDA and logistic regression. The two methods have very similar results, although the percentage of false negatives is slightly (but still noticably) higher for QDA. 
...
randforr_list_MSE.DF %>% ggplot() + 
  ...
  geom_point(aes(x=best.mtry, y = randforr_list_MSE[best.mtry]), shape = 10, col = "#e1923c", size = 40) +
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.background = element_blank(),
        legend.position = "none", plot.title = element_text(hjust = 0.5)) + 
  labs(x="Degrees", y = "Test MSE") + 
  ggtitle(expression(paste("Figure X: Test MSE by varying degrees"))) + 
  xlim(1, 15) + 
  ylim(0.14, 0.20)
```

In figure X we see the test MSE by varying degrees of variables used to make the tree. The lowest test MSE can be found when using 8 variables and the corresponding test MSE is `r round(min.testMSE, 2)`.
...
vip(randforr_best, aesthetics = list(fill = c("#d48149","#da9566","#ca6e2f","#ad5e28","#e1a983","#e8bda0","#2e1f18",
  ...
  labs(x = " ", y = "Relative influence") 
```
The results on figure X indicate that across all of the trees considered in the random forest, municipality is by far the most important variable when it comes to predicting the age of a property (old or new). Other important variables are number of garages, whether there is an elevator in the building and property worth. 
...
kbl(kabletableRF) %>% 
  kable_styling(full_width = F, position = "center") %>%
  add_header_above(c(" " = 1, "True values" = 2)) %>% 
  add_header_above(c("Table 4: Random forest " = 3),bold=T,font_size = 16)

randforr_MSE_best <- round(1-mean((pred_randforr_best==test.set2$AgeNew)),2)
```

Setja random forest í töfluna með boosting, lda og qda
