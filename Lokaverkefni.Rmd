...
par(xpd=TRUE)
corrplot.mixed(cor(da), lower.col = "#d88121", number.cex=0.7, tl.pos = "lt", upper.col = "#8c5e4a", tl.col="#341e12", tl.cex=0.7, tl.srt = 45, title = "Figure 7: Correlation plot of property elements", mar=c(0,0,4,0))
```

A correlation plot of 14 property elements can be seen on figure 7. Price of properties appears to have a moderately strong positive correlation with the size of properties (in square meters), year of sale and number of rooms, living rooms, floors, showers and toilets in a property. 
...
```
  ...
  Running factor analysis on the data set as a whole obtained 25 dimensions all with eigenvalues higher than one, indicating that the dimension accounts for more variance than one of the original variables. These 25 dimensions only collectively explained around 66% of data variance. The variable for neighbourhood had no correlation to any dimensions so it will be removed for further analysis, addition to price per square meter since it had high correlation to other predictors. 
...
The lasso approach found `r nrow(haha)` important coefficients. The model explained around `r r2train.lasso` of variance ($R^2$) in property price for the training data and around `r r2.lasso` for the test data. With $\lambda$ set to the optimal CV value, training error was around `r lasso.trainmse` m.ISK and test error around `r lassomse.` m.ISK. 
  ...
  In align with the factor analysis result the regions of the municipality variable do seem to work well as seperate predictors with differing influence regarding property price. Additionally in support of the factor analysis results number of toilets and bathtubs seem to have similar effect on property pricing and might be considered as a singular predictor.
  
  The main issue with this approach is blindness regarding correlated/confounding variables. As noted earlier (see Figure X.) square meter had the strongest influence on data variance. Here we see that variable is included but rather low, however all the variables from the first dimension of the factor analysis are also included here. Since Lasso is blind to their influence on each other it is important to keep that in mind and not exclude the effect that square meter might have on property price. 
