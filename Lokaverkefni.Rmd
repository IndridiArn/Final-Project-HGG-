---
title: "Nonlinear times at Ridgemont high"
...
## Introduction 
In this assignment we carry out an analysis of a dataset containing information of about 40 thousand real estates sold in Iceland from 2008 to 2018. The origin of the data can be traced to the National Registry of Iceland which performs an evaluation of properties worth every year. Current property worth will be referred to simply as property worth but its values were calculated based on property worth in 2019.
...
Figure 1 shows the distribution of property worth based on the municipality on one hand, and neighbourhood on the other. They are mostly right skewed. The highest valued properties on average are in Seltjarnarnes ($\mu \approx$ 52 m.ISK), Garðabær ($\mu \approx$ 49 m.ISK). The lowest in Hafnarfjörður ($\mu \approx$ 35 m.ISK) and Reykjavík ($\mu \approx$ 35 m.ISK). 

  The distribution for Seltjarnarnes differs from the rest in both graphs. It is the only variable that has a multimodal distribution of property worth compared to the other municipalities. It also has a flatter curve compared to the other municipalities. The property worth in the Flatir neighbourhood in Garðabær is the only left-skewed distribution.   
...
Figure 2 shows a gradual increase in both property worth as well as variability of property worth over the years. The average property worth over the years 2012 to 2018 is approximately 37 m.ISK. This 
...
Figure 3 shows property worth based on various elements. In general, these elements seem to have a positive linear relationship with property worth. However, number of living rooms, storage rooms and rooms, seem to be an exception as the relationship between these variables has a clear curve.
...
In this part the worth of properties (_ísl._ núvirði) will be predicted using the lasso and boosting method. 
Factor analysis will be performed on the dataset first in order to get a better idea of its nature.
...
Factor analysis on the data set as a whole obtained 25 dimensions all with eigenvalues higher than one, indicating that the dimension account for more variance than one of the original variables. These 25 dimensions only collectively explained around 66% of data variance. Neighbourhood had no correlation to any dimensions so it will be removed for further analysis. 
...
```
  ...
  Figure 9 shows the relative and cumulative variance explained by the factor analysis dimensions. Removing the two variables reduced the dimensions from 25 to seven and only decreased the cumulative variance explained by around 2%. According to the factor analysis the most influential variables are municipality, square meters, sales year, property worth, and number of bathtubs. 
  
  The first dimension consists of property worth, square meters, floors, toilets, rooms, living rooms, and property type (where square meters is clearly the most influental), together they might be representing property size. The second consists of age, parking, and elevator status. The third consists of number of bathtubs and showers, together might represent the number of master bathrooms. Sales year and storage appear to have their own dimension. The factor analysis showed that municipality had substantially higher contribution rate compared to the other variables and works as multiple seperate dimensions, two of which are included in the seven dimensions. 
...
data.frame(lambda = lasso.cv$lambda, 
  ...
  annotate(geom="text",y=10^(0.03+log10(bestmse)), x=10^log10(bestlam), label= expression(paste(lambda, "=1.29")))
```

Figure 10 shows the test MSE for a range of $\lambda$ values via cross-validation. The test MSE was optimal at `r round(bestlam,2)`. 
...
lasso.trainmseNew <- log10(round((mean(lasso.pred[,1])^2),2))
...
```
  ...
  Figure 11 shows the `r nrow(vi(lasso.mod))` predictors that Lasso deemed influental regarding property worth. The model explained around `r r2.lasso` of property worth variance ($R^2$). With training error around `r round(lasso.trainmseNew, 3)` and test error around `r round(lasso.testMSE, 3)`. 
...
In Figure 12 we see the relationship between lasso´s predicted values and observed values. The right graph includes all values in the data set and we appear to have an extreme value at the top of the plot. 
  The left graph is a close-up of all the values excluding the extreme value. The relationship seems stronger for properties worth less but starts to gradually worsen as the worth increases.
...
lasso <- com %>% ggplot(aes(Importance.x,Variable,
  ...
  annotate(geom="text",y="Bathtubs", x=2000, label= paste0("MSE= log10(", round(lasso.testMSE, 2),")"), size=3) + 
  annotate(geom="text",y="Age", x=2000, label= paste0("RMSE= log10(", round(lasso.RMSE, 2),")"), size=3)
...
In Figure 15 we can see the results from both the lasso approach and boosting. The MSE and RMSE for boosting is lower than it is for lasso. Additionally it includes 10 variables while Lasso has 19. The approaches do not seem to evaluate the variables the same way, the only one that seems consistantly influential is the sales year. 
  ...
  Boosting evaluated square meter as the most influential variable. Lasso did not null out the effect of this variable but still made it the second lowest coefficient of them all. Square meters had the highest correlation to the first dimension obtained by the factor analysis. From this we might assume that square meter is influential regarding property worth, but acts as a confounding variable. This is a weakpoint of the lasso approach. 
...
In figure 16 we see the test MSE by varying degrees of variables used to make the tree (500 trees were grown). The lowest test MSE can be found when using 8 variables and the corresponding test MSE is `r round(min.testMSE, 2)`. 
...
In this part of the analysis we carry out an analysis of a dataset containing information about real estates sold in Iceland in 2019 and 2020. The variable current property worth was calculated based on property worth in 2021. 
Only four variables in the new dataset overlapped with the variables our old dataset. They are square meters, year of sale, age of property and number of parking spaces. 

The model via boosting was chosen the best model and will be performed here to predict property worth on the new dataset. 
...
#bonus$Sqm <- as.numeric(bonus$Sqm)
bonus <- bonus %>%
  filter(bonus$year>2018)
bonus <- bonus %>% select(nuvirdi, ibm2,fjbilsk,age_studull, year)
var.labels <- c(nuvirdi = "Prop.Worth", ibm2 = "Sqm", fjbilsk = "Parking", age_studull = "Age", bath_fixtures = "Bathrooms", year = "Salesyear", hverfi = "Neighbourhood")
...
```{r}
datam <- data %>% select(Prop.Worth,Sqm,Parking,Age,Salesyear)
TRAIN <- sample(1:nrow(datam), nrow(datam)*0.7)
train.set <- datam[TRAIN,]
test.set <- datam[-TRAIN,]
...
# myGrid <- expand.grid(n.trees = c(100, 500), 
#                       interaction.depth = c(1,2,3,4,5), 
#                       shrinkage = c(0.001, 0.1, 0.125, 0.2), 
#                       n.minobsinnode = c(10))
set.seed(1)
# gbm_tree_tune <- train(Prop.Worth ~ ., data = train.set, 
#                        method = "gbm", 
#                        distribution = "gaussian", #### Tekur langan tíma að keyra þetta !
#                        trControl = trainctrl,     #### niðurstöður eru í myBestGrid2
#                        verbose = FALSE, 
#                        tuneGrid = myGrid)
# 
# 
# # which parameters turn out best? 
# gbm_tree_tune$bestTune


# only use the best parameters for the model
#myBestGrid <- gbm_tree_tune$bestTune

myBestGrid2 <- data.frame("n.trees" = 100,"interaction.depth"= 5,"shrinkage"=0.125,"n.minobsinnode"=10)
...
rmse.boost <- log10(sqrt(BOOST.test.MSE))
```
We ran cross-validation on multiple combinations of trees (using the old data set) using only the variables that overlapped in both data sets. Tuning the parameters resulted in a model with 100 boosting iterations (n.trees = 100), a max tree depth of five (interaction depth = 5), and a λ of 0.125. The minimum terminal node size was kept constant at 10.


```{r}
boost22 <- vi(gbm_tree_final)
booooo <- merge(boos, boost22, by="Variable")
boostbonus <- booooo %>% ggplot(aes(Importance.y,Variable,
                            fill=Variable)) +
  geom_bar(stat = "identity")+ 
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), 
        panel.background = element_blank(), legend.position = "none") +
  scale_fill_manual(values=sample(ColorsDT$Color)) + labs(x="Relative Influence", y=" ") + ggtitle("Boosting 2019-2020")
boostfyrri <- booooo %>% ggplot(aes(Importance.x,Variable,
                            fill=Variable)) +
  geom_bar(stat = "identity")+ 
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), 
        panel.background = element_blank(), legend.position = "none",
        ...
        axis.text.y=element_blank(),
        axis.ticks.y=element_blank()) +
  scale_fill_manual(values=sample(ColorsDT$Color)) + labs(x="Relative Influence", y=" ") + ggtitle("Boosting 2012-2018") 
grid.arrange(boostbonus,boostfyrri, nrow=1, top="Figure 18: Boosting comparison") 
```

As shown in the left plot on figure 18, there are three variables out of the four that have higher relative importance. They are square meters of property, the year the property was bought and the age of the property. Square meters per property is by far the most important variable, which aligns with the results on the right plot. Overall, the model via boosting gives very similar results on the new dataset from 2021. 
