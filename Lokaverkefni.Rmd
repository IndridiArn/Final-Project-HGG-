...
y1 <- data %>%
  ...
  ggplot() + geom_bar(aes(y=nuvirdi,x=yearkeypt, fill=yearkeypt), stat="identity", position = "dodge") +
  scale_fill_manual(values=ColorsDT$Color) +
  ...
  geom_hline(yintercept=mean(data$nuvirdi), col = '#7b4912', size=1) + labs(x= "Sales year", y= "Mean Property Price") + coord_flip()
y2 <- data %>% ggplot(aes(x=nuvirdi,y=yearkeypt, fill=yearkeypt)) +
  geom_density_ridges_gradient() +
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.background = element_blank(), legend.position = "none") + 
  ggtitle(" ") + xlim(c(0,100000)) + scale_fill_manual(values=ColorsDT$Color) + 
  labs(y = " ", x = "Property Price")
grid.arrange(y1,y2,nrow=1, top = "Figure 2: Property price by year of sale")
```

In figure 2 we can see the mean property price by year of sale. The vertical line represents the mean property price over the years 2012 to 2018 ($\mu \approx$ `r round(mean(data$nuvirdi))`). Property prices have gradually increased over the years.  
...
colnames(da) <- c("Pr.Price","Area","Floors","Parking","Baths","Showers","Toilets","Kitchens","Rooms","Liv.r.","Age", "Storage","Sqm.Price")
corrplot.mixed(cor(da[-12]), lower.col = "#d88121", number.cex=0.7, tl.pos = "lt", upper.col = "#8c5e4a", order="hclust", tl.col="#341e12", tl.cex=0.7, tl.srt = 45, 
               title = "Figure 6: Correlation plot of property attributes", mar=c(0,0,4,0))
```

LAGA - label cutoffs
ATH, vantar AGE inn í - ef þú skoðar _da_ gagnasettið þá sérðu að Age tekur bara gildi frá 0-8 (en í _data_ gagnasettinu er hæsta gildið 177). Veit ekki alveg hvar þetta gæti hafa gerst í kóðanum.  
...
Our optimal value of λ is 0.0001 for minimum MSE.
