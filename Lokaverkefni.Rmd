...
data.frame(lambda = lasso.cv$lambda, 
  ...
  annotate(geom="text",y=10^8.1438, x=10^-0.5, label= expression(paste(lambda, "=0.36")))



```

In figure 8 we can see the test MSE for different lambda values via cross-validation. Our optimal value of $\lambda$ is log10(`r round(bestlam,2)`), for minimum test MSE. 
...
haha <- plot[-1,] %>%
  filter(value>0) %>%
  arrange(-value)
#rownames(haha) <- c("Seltjarnarnes", "Prop. Type", "Toilets", "Kitchens", "Garðabær", "Parking", "Hafnarfjörður","Living rooms", "Showers","Sales Year", "Rooms", "Bathtubs", "Floors", "Elevator") #keyra coef(lasso.mod) til að sjá hvaða rownames eiga að vera hér
haha %>%
  ggplot(aes(value,reorder(rownames(haha),value), col=coef)) + geom_point(show.legend = F, size = 3)+ 
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), 
        panel.background = element_blank(), legend.position = "none") + 
  scale_fill_manual(values = sample(ColorsDT$Color)) + 
  scale_color_manual(name = "Type of property",values=sample(ColorsDT$Color)) + labs(x="Coefficient", y=" ") + ggtitle(expression(paste("Figure 9: Predictors with non-zero Lasso coefficients with ", lambda, " = 10")))
...
LOGmin.testMSE <- log10(min(test.MSE)) 
```

In figure 11 we have computed the test error as a function of the shrinkage value. Our optimal $\lambda$ is `r min.lambda` for the minimum test MSE. With lambda set to log(10) the test MSE is `r round(LOGmin.testMSE, 2)` m.isk. 
