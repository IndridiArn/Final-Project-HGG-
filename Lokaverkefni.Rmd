...
```
LAGA: Túlkun : TODO ATH 
In figure 14 we see a plot of the RMSE via cross-validation according to various measures of tree size and shrinkage value. We can see here out of the two options.

Now we'll use the training model to predict the worth of a property for the test data and see the test MSE. 
```{r}
library(vip)
...
As shown in figure X, there are four variables that have high relative importance. They are square meters of property, the year the property was bought, the age of the property and at last, municipality the property is in. Square meters per property is by far the most important variable. In figures X and X in part I we learned that the more square meters are in a property the more expensive it is. We also learned that property prices increase the younger the property is. 

The test set MSE which is `r BOOST.test.MSE`. 


LAGA ÞESSA TÖFLU. BERGRÓS, gætir þú tékkað á hvort hún sé rétt? 
```{r}
...
Here we will predict whether a property is old or new. We've created a new binary variable: If the property age exceeds the overall median age (32 years) it is categorised as an old property, the remaining as a new. 
